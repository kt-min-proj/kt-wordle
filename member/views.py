...
def signup_custom(request):
    classes = ['수도권1반', '수도권2반', '수도권3반', '수도권4반', '수도권5반', 
                    '강원권1반', '충남/충북권1반', '대구/경북권1반', '전남/전북권1반', '부산/경남권1반']
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not PasswordHasher().verify(password, user_pw.encode()):
                return render(request, 'member/login.html', { 'login_status': 'ID 혹은 PASSWORD를 확인해주세요.' })
